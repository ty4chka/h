...
try:
    import yandex_music
    import yandex_music_exceptions
    ...
    class FakeException:
        class NotFoundError(Exception):
            pass
    yandex_music_exceptions = FakeException()
else:
    yandex_music_exceptions = yandex_music_exceptions
...
class YaMusicModule(ModuleBase):
    ...
    async def on_load(self) -> None:
        if not YANDEX_MUSIC_AVAILABLE:
            self.log.error("yandex_music not installed. Run: pip install git+https://github.com/MarshalX/yandex-music-api")
            return
        await super().on_load()
    ...
    async def cmd_ylyrics(self, event: telethon.types.Message) -> None:
        ...
        except yandex_music_exceptions.NotFoundError:
            await event.edit(
...
_mcub_inst = YaMusicModule()
_mcub_inst._live_module_configs = {}
async def yguide_handler(event):
    ...
    await _mcub_inst.cmd_yguide(event)

async def ybio_handler(event):
    _safe_edit_patch(event)
    if not getattr(_mcub_inst, 'client', None):
        _mcub_inst._hydra_client_cache = getattr(event, 'client', None)
        _mcub_inst.client = getattr(event, 'client', None)
        try:
            ...
            import logging
            logging.getLogger('mcub_oop').error(f'on_load error: {_e}')
    await _mcub_inst.cmd_ybio(event)

async def ysearch_handler(event):
    _safe_edit_patch(event)
    if not getattr(_mcub_inst, 'client', None):
        _mcub_inst._hydra_client_cache = getattr(event, 'client', None)
        _mcub_inst.client = getattr(event, 'client', None)
        try:
            ...
            import logging
            logging.getLogger('mcub_oop').error(f'on_load error: {_e}')
    await _mcub_inst.cmd_ysearch(event)

async def ynow_handler(event):
    _safe_edit_patch(event)
    if not getattr(_mcub_inst, 'client', None):
        _mcub_inst._hydra_client_cache = getattr(event, 'client', None)
        _mcub_inst.client = getattr(event, 'client', None)
        try:
            ...
            import logging
            logging.getLogger('mcub_oop').error(f'on_load error: {_e}')
    await _mcub_inst.cmd_ynow(event)

async def ynowt_handler(event):
    _safe_edit_patch(event)
    if not getattr(_mcub_inst, 'client', None):
        _mcub_inst._hydra_client_cache = getattr(event, 'client', None)
        _mcub_inst.client = getattr(event, 'client', None)
        try:
            ...
            import logging
            logging.getLogger('mcub_oop').error(f'on_load error: {_e}')
    await _mcub_inst.cmd_ynowt(event)
...
async def ylike_handler(event):
    ...
    if not getattr(_mcub_inst, 'client', None):
        ...
        try:
            ...
            import logging
            logging.getLogger('mcub_oop').error(f'on_load error: {_e}')
    await _mcub_inst.cmd_ylike(event)

async def yunlike_handler(event):
    _safe_edit_patch(event)
    if not getattr(_mcub_inst, 'client', None):
        _mcub_inst._hydra_client_cache = getattr(event, 'client', None)
        _mcub_inst.client = getattr(event, 'client', None)
        try:
            ...
            import logging
            logging.getLogger('mcub_oop').error(f'on_load error: {_e}')
    await _mcub_inst.cmd_yunlike(event)
...
async def ydislike_handler(event):
    ...
    if not getattr(_mcub_inst, 'client', None):
        ...
        try:
            ...
            import logging
            logging.getLogger('mcub_oop').error(f'on_load error: {_e}')
    await _mcub_inst.cmd_ydislike(event)

async def ylyrics_handler(event):
    _safe_edit_patch(event)
    if not getattr(_mcub_inst, 'client', None):
        _mcub_inst._hydra_client_cache = getattr(event, 'client', None)
        _mcub_inst.client = getattr(event, 'client', None)
        try:
            ...
            import logging
            logging.getLogger('mcub_oop').error(f'on_load error: {_e}')
    await _mcub_inst.cmd_ylyrics(event)

modules_help = { 'yamusic': {"yguide": "Команда yguide", "ybio": "Команда ybio", "ysearch": "Команда ysearch", "ynow": "Команда ynow", "ynowt": "Команда ynowt", "ylike": "Команда ylike", "yunlike": "Команда yunlike", "ydislike": "Команда ydislike", "ylyrics": "Команда ylyrics"} }
