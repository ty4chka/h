...
func (e Editor) renderSidebar() string {
	...
	if e.Mode == ModePalette {
		for i, tmpl := range DefaultTemplates {
			cursor := "  "
			if i == e.PaletteCursor { cursor = "▶ " }
			if i == e.PaletteCursor {
				b.WriteString(selectedStyle.Render(cursor + tmpl.Title))
			} else {
				b.WriteString(style.Render(cursor + tmpl.Title))
			}
			b.WriteString("\n")
		}
	} else {
		for _, block := range e.Blocks {
			marker := "  "
			if block.Selected { marker = "▶ " }
			b.WriteString(lipgloss.NewStyle().Foreground(lipgloss.Color(block.Color)).Render(marker + "█ " + block.Title))
			b.WriteString("\n")
		}
	}
	b.WriteString(borderStyle.Render(strings.Repeat("─", w))); b.WriteString("\n")
	if e.Mode == ModeBlockSettings && e.SettingsBlock != nil {
		b.WriteString(titleStyle.Render("Settings")); b.WriteString("\n")
		keys := make([]string, 0, len(e.SettingsBlock.Data))
		for k := range e.SettingsBlock.Data { keys = append(keys, k) }
		for i, k := range keys {
			cursor := "  "
			if i == e.SettingsCursor { cursor = "▶ " }
			val := e.SettingsBlock.Data[k]
			if len(val) > 12 { val = val[:12] + "..." }
			b.WriteString(style.Render(fmt.Sprintf("%s%s: %s", cursor, k, val)))
			b.WriteString("\n")
		}
	} else {
		b.WriteString(style.Render("Selected: "))
		found := false
		for _, block := range e.Blocks {
			if block.Selected {
				b.WriteString(lipgloss.NewStyle().Foreground(lipgloss.Color(block.Color)).Render(block.Title))
				found = true
				break
			}
		}
		if !found { b.WriteString(style.Render("none")) }
		b.WriteString("\n")
		b.WriteString(style.Render(fmt.Sprintf("Blocks: %d", len(e.Blocks))))
		b.WriteString("\n")
		b.WriteString(style.Render(fmt.Sprintf("Conns: %d", len(e.Connections))))
		b.WriteString("\n")
	}
